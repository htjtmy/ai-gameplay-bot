...
class PerformanceTester:
    ...
    def __init__(self, transformer_url="http://localhost:5001"):
        """
    ...
    def save_results(self, results: Dict, output_dir: str = "results"):
        ...
        with open(txt_path, 'w') as f:
            ...
            if 'latency' in results.get('transformer', {}):
                tr_stats = results['transformer']['latency']
                f.write("NEURAL NETWORK MODEL\n")
                f.write("-" * 60 + "\n")
                f.write(f"Average Latency:     {tr_stats['avg_latency']:.4f} seconds\n")
                f.write(f"Median Latency:      {tr_stats['median_latency']:.4f} seconds\n")
                f.write(f"Min Latency:         {tr_stats['min_latency']:.4f} seconds\n")
                f.write(f"Max Latency:         {tr_stats['max_latency']:.4f} seconds\n")
                f.write(f"P95 Latency:         {tr_stats['p95_latency']:.4f} seconds\n")
                f.write(f"P99 Latency:         {tr_stats['p99_latency']:.4f} seconds\n")
                f.write(f"Success Rate:        {tr_stats['success_rate']:.2f}%\n\n")
            ...
            if 'load' in results.get('transformer', {}):
                ...
                f.write("-" * 60 + "\n")
                f.write(f"Transformer Requests/Second:   {tf_load['requests_per_second']:.2f}\n")
