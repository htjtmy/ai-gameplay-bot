...
# Use uv-run python inside venv (unless conda env is active)
ifneq ($(CONDA_PREFIX),)
RUN_PYTHON := python
else
...
venv: install-uv
ifneq ($(CONDA_PREFIX),)
	@echo "Conda environment detected ($(CONDA_PREFIX)); skipping uv venv creation."
else
	@if [ -d "$(VENV_DIR)" ]; then \
...
dev: setup data
	...
	@echo "Training transformer model with reduced epochs for faster testing..."
	@make run-control
