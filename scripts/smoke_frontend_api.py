...
Inference Lab
- POST /api/test_predict (transformer)
...
It also:
- Quick-trains Transformer weights (fast)
- Spawns deploy_transformer.py, control_backend.py
...
def test_inference_lab() -> None:
    ...
    # /api/test_predict for transformer model
...
def test_model_repository() -> None:
    ...
    # upload a model (use Transformer weights as a test artifact)
    if not TR_WEIGHTS.exists():
        raise RuntimeError(f"Missing weights to upload: {TR_WEIGHTS}")

    meta = {"uploaded_by": "smoke_frontend_api", "ts": int(time.time())}
    files = {
        "file": (f"smoke_upload_transformer_{int(time.time())}.pth", TR_WEIGHTS.read_bytes(), "application/octet-stream"),
    ...
    r = _post_json("/api/load_model", {"model_type": "transformer", "filename": uploaded_filename}, timeout=20)
    body = _assert_status(r, 200, "POST /api/load_model (transformer)")
    print("[OK] /api/load_model (transformer) ->", body)

    # verify transformer still predicts after reload
    r = _post_json("/api/test_predict", {"model": "transformer"}, timeout=15)
    _assert_status(r, 200, "POST /api/test_predict transformer after load_model")
    print("[OK] /api/test_predict transformer after load_model")

    # delete the uploaded file (should not be the active weights file; backend blocks deleting active)
    r = requests.delete(
        f"{CB_URL}/api/delete_model",
        json={"type": "transformer", "filename": uploaded_filename},
        timeout=10,
    )
...
def test_dojo_capture_and_training() -> None:
    ...
    r = _post_json("/api/train_offline", {"dataset": session_id, "model_type": "transformer", "epochs": 1}, timeout=10)
