...
# Create logs directory if it doesn't exist / 创建日志目录（如不存在）
...
class ColoredFormatter(logging.Formatter):
    ...
    COLORS = {
        ...
        'CRITICAL': '\033[35m',  # Magenta / 紫红色
        'RESET': '\033[0m'  # Reset / 重置
    }

    def format(self, record):
        """Format the log record with colors. / 使用颜色格式化日志记录"""
        if sys.stdout.isatty():  # Only use colors for terminal output / 仅在终端输出时使用颜色
