...
def unified_predictor(
    ...
    """
    Unified predictor that uses the Transformer model.
    """
    return get_action_from_transformer(state, timeout=timeout)


# Optional helpers (useful in prod to check readiness)
def is_service_healthy(base_url: str = TRANSFORMER_BASE_URL, timeout: float = 2.0) -> bool:
    try:
...
def _main() -> int:
    ...
    try:
        ...
        action_tr = unified_predictor(example_state)
